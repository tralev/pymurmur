...
- **Citation** — provenance in `roadmap.md`
...
4. **Occlusion doesn't occlude** — ~~all neighbours marked visible, Θ is linear sum not probabilistic union~~ **FIXED (P1.1–P1.5)**
...
**Completed:** P1.1–P1.10 ✅ — ALL COMPLETE
...
**Completed:** P2.1–P2.10 ✅ — ALL COMPLETE (1,184 tests passing, 0 failures)
...
but documented here as a known upgrade path. See `roadmap.md` §9.
in `roadmap.md` if scope changes.
...
- [x] **Import cycle dead** — `physics/flock` does not import anything from `physics/forces/` (AST check)
- [x] **Preset domains** — All `conf/*.yaml` load with correct `domain.width` not overwritten by `capture.width`
- [x] **Nested config round-trip** — `SimConfig.from_file(p) → to_file → from_file` produces identical config
- [x] **Unknown keys warn** — Extra YAML keys produce `UserWarning`
- [x] **Depth validation** — `validate()` rejects `depth=0`
...
- [x] **Mode step respects active mask** — Holey-flock test: inactive rows unchanged
- [x] **Mode determinism** — Same seed → identical flock state after 100 steps per mode
- [x] **KDTreeIndex global indices** — Query returns capacity-space row numbers (not compacted)
- [x] **KDTreeIndex boxsize** — Toroidal queries correct across seam
- [x] **SpatialHashGrid ghost-cell** — Cross-seam neighbour matches full brute-force
- [x] **ExtensionManager** — Empty → no-op; pre_step order verified (extensions before mode)
- [x] **Holey-mask matrix** — All modes × 20 steps, no exceptions, inactive rows unchanged
- [x] **VAO rebuild** — After buffer growth (`add_boids` beyond chunk), VAO rebuilt and renders correctly
...
- [x] **ForceTerm conformance** — `composeForces(a+b) = composeForces(a) + composeForces(b)`
- [x] **ForceTerm toggles** — Flipping `term.enabled = False` mid-run → that term contributes zero
- [x] **Architecture test** — `ALLOWED_EDGES` extended: flock, forces/*, extensions, engine, viz, capture, analysis
- [x] **Test command** — `pytest test/core/test_config.py test/physics/forces/test_mode_contract.py test/physics/test_spatial_index_contract.py test/physics/extensions/test_extensions.py test/physics/test_composition.py test/simulation/test_engine.py test/viz/test_renderer.py -v`
...
These were identified by cross-referencing roadmap_deepseek.md against the audit files in `roadmap.md`:

| # | Item | New Location | Source |
|---|---|---|---|
| 1 | Grid-mode separation normalization | **P3.11** | `roadmap.md` §2.3 |
| 2 | 1.45·R_blob floating boundary for field mode | **P3.12** | `roadmap.md` §8 |
| 3 | Functional ForceTerm + composeForces composition | **P2.10** | `roadmap.md` §8, `roadmap.md` §15 |
| 4 | Position init variants (sphere_shell, grid, gaussian, box, blob) | **P0.15** | `roadmap.md`, `roadmap.md` §12 |
| 5 | Theme material tables (ambient+diffuse pairs) | **P8.5** | `roadmap.md` §4 |
| 6 | Altitude-deviation reward metric | **P9.8** | `roadmap.md` §2 |
| 7 | Zero-speed deterministic fallback `(minSpeed, 0, 0)` | **P0.9** | `roadmap.md` §4.4 |
| 8 | Desktop pilotable-flock mode | **P7.6** | `roadmap.md` §7.2 |
| 9 | Multi-flock support (reconsidered in Appendix A) | **Appx A** | `roadmap.md` §9 |
...
*Generated from a full cross-reference of `roadmap.md` and `roadmap_deepseek.md` against the audit corpus, July 2026.*
...
| P0 | P0.11 | Capability probing | L3 | `pymurmur/__main__.py` | — | `roadmap.md`. |
| P0 | P0.12 | Math helpers in core/types.py | L0 | `core/types.py`. **Tests:** `test/core/test_types.py` | `test/core/test_types.py` | `todo_claude.md` §3 (Rodrigues), `roadmap.md`analysis/metrics.py::compute_h2` | — | `todo_claude.md` §10. |
...
| P0 | P0.15 | Position init variants | L0 | `physics/boid.py`. **Tests:** `test/physics/test_boid.py` | `test/physics/test_boid.py` | `roadmap.md` position init, `sc |
...
| P1 | P1.7 | Force kernel fixes | L0 | `physics/forces/_base.py`. **Tests:** `test/physics/forces/t... | `test/physics/forces/test_kernels.py` | `git0` W3.1, `roadmap.md` § |
| P1 | P1.8 | Vicsek update: memory term + tangent-plane noise | L0 | `physics/forces/vicsek.py`. **Tests:** `test/physics/forces/... | `test/physics/forces/test_vicsek_core.py` | `git2` R1, `roadmap.md` §1. |
...
| P2 | P2.5 | Ghost-cell replication + modulo cells | L1 | `physics/flock.py::SpatialHashGrid` | — | `git3` R2, `roadmap.md` §13 |
...
| P2 | P2.10 | Functional ForceTerm composition | L1 | `physics/forces/_base.py` (extend). **Tests:** `test/physics... | `test/physics/forces/test_force_terms.py` | `roadmap.md` §8 (composeForces p |
| P3 | P3.1 | Wander path (boundedUnitTravel) | L1 | physics/forces/field.py; physics/extensions/predator.py; phy... | `‖path‖ ≤ 1` for 10⁶ fuzzed t; heading continuous (‖h(t+ε)−h... | `git6` R2, `roadmap.md` §8. |
| P3 | P3.2 | Five Lissajous blob anchors + cyclic phase weights | L1 | `physics/forces/field.py::FieldMode` | 2K birds → k-means finds ≥4 clusters at t=30s; per-bird targ... | `git6` R3, `roadmap.md` §2. |
| P3 | P3.3 | Leader/chaser groups | L1 | physics/forces/field.py; physics/extensions/predator.py; phy... | test/physics/forces/test_field.py; test/physics/extensions/t... | `git6` R4, `roadmap.md` §2. |
| P3 | P3.4 | Shell force + inner cavity | L1 | physics/forces/field.py; physics/extensions/predator.py; phy... | Settled 5K blob — centre voxel density < 0.3× shell band den... | `git6` R5, `roadmap.md` §2. |
| P3 | P3.5 | Slot repulsion | L1 | physics/forces/field.py; physics/extensions/predator.py; phy... | kernel zero at r_slot and beyond; continuous at boundary | `git6` R6, `roadmap.md` §3. |
| P3 | P3.6 | Remaining 6 field terms | L1 | physics/forces/field.py; physics/extensions/predator.py; phy... | test/physics/forces/test_field.py; test/physics/extensions/t... | `git6` R6, `roadmap.md` §2. |
| P3 | P3.7 | Ripple envelopes (vectorised) | L1 | physics/forces/field.py; physics/extensions/predator.py; phy... | test/physics/forces/test_field.py; test/physics/extensions/t... | `git6` R7, `roadmap.md` §2. |
| P3 | P3.8 | Bounded panic + blackening | L1 | physics/forces/field.py; physics/extensions/predator.py; phy... | panicked speed ≤ 2.35·v0 always; near-threat pair separation... | `git6` R8, `roadmap.md` §3. |
| P3 | P3.9 | Threat FSM + force bundle | L1 | physics/forces/field.py; physics/extensions/predator.py; phy... | test/physics/forces/test_field.py; test/physics/extensions/t... | `git6` R9, `roadmap.md` §4. |
| P3 | P3.10 | Blob init + 7 field presets | L1 | physics/forces/field.py; physics/extensions/predator.py; phy... | All 7 presets load with documented values; frame-0 lobes vis... | `git6` R12, `roadmap.md` §1 |
| P3 | P3.11 | Grid-mode separation normalization | L1 | `physics/forces/field.py` | test/physics/forces/test_field.py; test/physics/extensions/t... | `roadmap.md` §2.3 ("Separat |
| P3 | P3.12 | Field-mode 1.45·R_blob floating boundary | L1 | `physics/forces/field.py::FieldMode` | test/physics/forces/test_field.py; test/physics/extensions/t... | `roadmap.md` §8, `roadmap.md`git5` R1, `roadmap.md` §4. |
| P4 | P4.2 | Force accumulation order | L1 | physics/forces/spatial.py; physics/forces/_kernels.py; analy... | test/physics/forces/test_spatial_variants.py; speed_real = k... | `git3` R1, `roadmap.md` §1, |
| P4 | P4.3 | Predator boids (species) | L1 | physics/forces/spatial.py; physics/forces/_kernels.py; analy... | test/physics/forces/test_spatial_variants.py; speed_real = k... | `git3` R3, `roadmap.md` §2. |
| P4 | P4.4 | Physical metrics | L1 | `analysis/metrics.py` — add `speed_real, accel_real, force_r... | Hand-set v → `speed_real = k_v·|v|` exactly; E ≈ P̄·elapsed ... | `git5` R3, `roadmap.md` §3. |
| P4 | P4.5 | Per-frame parameter jitter | L1 | physics/forces/spatial.py; physics/forces/_kernels.py; analy... | spacing_std(on) > spacing_std(off), same seed; config unchan... | `git5` R4, `roadmap.md` §5. |
| P4 | P4.6 | Parallel two-phase update | L1 | physics/forces/spatial.py; physics/forces/_kernels.py; analy... | test/physics/forces/test_spatial_variants.py; speed_real = k... | `git3` R4, `roadmap.md` §4. |
| P4 | P4.7 | Sphere centring + asymptotic wall | L1 | physics/forces/spatial.py; physics/forces/_kernels.py; analy... | test/physics/forces/test_spatial_variants.py; speed_real = k... | `git5` R2, `roadmap.md` §2. |
| P4 | P4.8 | Ecology completion | L1 | physics/forces/spatial.py; physics/forces/_kernels.py; analy... | test/physics/forces/test_spatial_variants.py; speed_real = k... | `todo_claude.md` §5–8, `roadmap.md`todo_claude.md` E12, `roadmap.md`physics/forces/_kernels.py` (new) | test/physics/forces/test_spatial_variants.py; speed_real = k... | `arch.md` §9, `roadmap.md`  |
| P5 | P5.1 | Steering core | L1 | physics/forces/angle.py; test/physics/forces/test_angle.py | 180° turn completes in π/rate seconds ±1 frame; per-frame he... | `git4` R1, `roadmap.md` §1. |
| P5 | P5.2 | Unified neighbour modes | L1 | physics/forces/angle.py; test/physics/forces/test_angle.py | boid_size | `git4` R2, `roadmap.md` §2. |
| P5 | P5.3 | Adaptive speed | L1 | physics/forces/angle.py; test/physics/forces/test_angle.py | m=0 → base+35 (linear); m≥7 → base; median 7th-NN distance c... | `git4` R3, `roadmap.md` §3. |
| P5 | P5.4 | Edge handling | L1 | physics/forces/angle.py; test/physics/forces/test_angle.py | boid_size | `git4` R4, `roadmap.md` §4. |
| P5 | P5.5 | Heading jitter | L1 | physics/forces/angle.py; test/physics/forces/test_angle.py | boid_size | `git4` R5, `roadmap.md` §5. |
| P5 | P5.6 | Incremental spatial grid | L1 | physics/forces/angle.py; test/physics/forces/test_angle.py | Neighbour sets == full-rebuild sets over 500 random-walk fra... | `git4` R6, `roadmap.md` §6. |
| P5 | P5.7 | Body-unit scale invariance | L1 | physics/forces/angle.py; test/physics/forces/test_angle.py | Doubling `boid_size` doubles all three thresholds; 2×-scale ... | `git4` R8, `roadmap.md` §10 |
| P6 | P6.1 | Fear-weighted alignment | L1 | physics/forces/vicsek.py | Stationary predator at centre → prey ⟨û·r̂⟩ > 0.8 within 5 s... | `git2` R2, `roadmap.md` §2. |
| P6 | P6.2 | Predator agent | L1 | physics/forces/vicsek.py | Monotone pursuit (≥90% of steps close distance); n_prey=0 → ... | `git2` R3, `roadmap.md` §2. |
| P6 | P6.3 | Asymmetric position collisions | L1 | physics/forces/vicsek.py | test/physics/forces/test_vicsek_species.py | `git2` R5, `roadmap.md` §3. |
| P7 | P7.1 | Persistent tick + Lissajous target | L1 | physics/forces/influencer.py; physics/forces/influencer.py::... | `T(t)` at t∈{0,970,2170} equals hand values; in-domain for s... | `git1` R1–R2, `roadmap.md`  |
| P7 | P7.2 | Move-then-steer at unit speed | L1 | physics/forces/influencer.py; physics/forces/influencer.py::... | Frozen target → convergence to hover/orbit; one-step lag aft... | `git1` R3, `roadmap.md` §1. |
| P7 | P7.3 | Rank-by-target-distance influence | L1 | physics/forces/influencer.py; physics/forces/influencer.py::... | test/physics/forces/test_influencer.py; T(t) | `git1` R4, `roadmap.md` §3. |
| P7 | P7.4 | Density-scaled init | L1 | physics/forces/influencer.py; physics/forces/influencer.py::... | Init density equal across N∈{100,1000,8000} (±10%); frame-0 ... | `git1` R5, `roadmap.md` §5. |
| P7 | P7.5 | Distance diagnostics | L1 | physics/forces/influencer.py; physics/forces/influencer.py::... | test/physics/forces/test_influencer.py; T(t) | `git1` R6, `roadmap.md` §6. |
| P7 | P7.6 | Desktop pilotable-flock mode | L1 | physics/forces/influencer.py; physics/forces/influencer.py::... | test/physics/forces/test_influencer.py; T(t) | `roadmap.md` §7.2 (pilot-aw |
| P8 | P8.1 | Sphere impostors + speed-stretched ellipsoid | L2 | viz/renderer.py; viz/shaders.py; viz/camera.py; viz/hud.py; ... | Centre pixel brighter than rim; corners = background | `git6` R10, `roadmap.md` §6 |
| P8 | P8.2 | Depth cues + Fresnel rim | L2 | viz/renderer.py; viz/shaders.py; viz/camera.py; viz/hud.py; ... | Near bird renders larger & more opaque than far | `git6` R10, `roadmap.md` §4 |
| P8 | P8.3 | Trail rendering ×4 | L2 | `viz/trails.py` (new), `physics/flock.py` (position_history) | test/viz/test_renderer.py; test/viz/test_trails.py; test/viz... | `git6` R10, `roadmap.md` §9 |
| P8 | P8.4 | Winged flapping mesh + gradient sky | L2 | viz/renderer.py; viz/shaders.py; viz/camera.py; viz/hud.py; ... | test/viz/test_renderer.py; test/viz/test_trails.py; test/viz... | `git5` R6–R7, `roadmap.md`  |
| P8 | P8.5 | Colour channels + theme wiring | L2 | viz/renderer.py; viz/shaders.py; viz/camera.py; viz/hud.py; ... | test/viz/test_renderer.py; test/viz/test_trails.py; test/viz... | `git6` R10, `roadmap.md` §1 |
| P8 | P8.6 | Adaptive quality wired | L2 | `analysis/perf.py` (governor logic), `viz/visualizer.py` (co... | test/viz/test_renderer.py; test/viz/test_trails.py; test/viz... | `git6` R11, `roadmap.md` §1 |
...
| P8 | P8.8 | Dual-view + orthographic presets | L2 | viz/renderer.py; viz/shaders.py; viz/camera.py; viz/hud.py; ... | test/viz/test_renderer.py; test/viz/test_trails.py; test/viz... | `roadmap.md` §5. |
| P8 | P8.9 | GPU-free matplotlib fallback | L2 | `capture/mpl_recorder.py` (new) | test/viz/test_renderer.py; test/viz/test_trails.py; test/viz... | `roadmap.md` §5. |
| P8 | P8.10 | Fixed-timestep accumulator + interpolation | L2 | viz/renderer.py; viz/shaders.py; viz/camera.py; viz/hud.py; ... | 30fps vs 60fps → identical physics after same elapsed time | `roadmap.md` §11, `roadmap.md`roadmap.md` §8. |
| P9 | P9.1 | Nematic order parameter | L1 | analysis/metrics.py; analysis/rewards.py; analysis/phase_dia... | test/analysis/test_metrics.py; test/analysis/test_metrics_in... | `git2` R7, `roadmap.md` §4. |
| P9 | P9.2 | MSD(τ) curve | L1 | analysis/metrics.py; analysis/rewards.py; analysis/phase_dia... | test/analysis/test_metrics.py; test/analysis/test_metrics_in... | `git2` R8, `roadmap.md` §4. |
...
| P9 | P9.8 | Motion metrics | L1 | analysis/metrics.py; analysis/rewards.py; analysis/phase_dia... | test/analysis/test_metrics.py; test/analysis/test_metrics_in... | `todo_claude.md` §15, `roadmap.md`analysis/rewards.py` | test/analysis/test_metrics.py; test/analysis/test_metrics_in... | `roadmap.md` §2. |
...
| P10 | P10.3 | Slider HUD | L2 | analysis/presets.py; viz/input_control.py; viz/hud.py | test/viz/test_input.py; test/test_cli.py; test/test_facade.p... | `roadmap.md` §6. |
| P10 | P10.4 | Cursor-ray spawning | L2 | analysis/presets.py; viz/input_control.py; viz/hud.py | test/viz/test_input.py; test/test_cli.py; test/test_facade.p... | `roadmap.md` §7. |
| P10 | P10.5 | CLI + facade | L3 | analysis/presets.py; viz/input_control.py; viz/hud.py | test/viz/test_input.py; test/test_cli.py; test/test_facade.p... | `roadmap.md` §6–8. |
...
| P11 | P11.1 | SSGA fidelity + uniform crossover | L2 | analysis/evoflock.py; physics/obstacles.py; physics/obstacle... | test/analysis/test_evoflock.py; @slow | `roadmap.md` W8. |
| P11 | P11.2 | Worst-of-4 evaluation | L2 | analysis/evoflock.py; physics/obstacles.py; physics/obstacle... | Monkeypatched objectives [0.9,0.8,0.95,0.7] → fitness 0.7; s... | `roadmap.md` W8. |
| P11 | P11.3 | Objectives | L2 | analysis/evoflock.py; physics/obstacles.py; physics/obstacle... | Trapezoid pinned at d/body∈{1.9→0, 2.5→1, 4→1, 5→0}; helix κ... | `roadmap.md` W8. |
| P11 | P11.4 | SDF obstacle layer | L1 | `physics/obstacles.py::ObstacleScene` | test/analysis/test_evoflock.py; @slow | `roadmap.md` W8. |
| P11 | P11.5 | Expanded gene set | L2 | analysis/evoflock.py; physics/obstacles.py; physics/obstacle... | test/analysis/test_evoflock.py; @slow | `roadmap.md` W8. |
| P11 | P11.6 | Protocol | L2 | analysis/evoflock.py; physics/obstacles.py; physics/obstacle... | test/analysis/test_evoflock.py; @slow | `roadmap.md` W8. |
| P12 | P12.1 | "marl" mode: deferred global rules | L1 | physics/forces/marl.py; analysis/gym_env.py; analysis/gym_en... | test/analysis/test_marl.py; pytest.importorskip("gymnasium")... | `roadmap.md` §1. |
| P12 | P12.2 | Gymnasium wrapper | L2 | `analysis/gym_env.py::MurmurationEnv` | test/analysis/test_marl.py; pytest.importorskip("gymnasium")... | `roadmap.md` §1. |
| P12 | P12.3 | Scripts (dependency-gated) | L3 | physics/forces/marl.py; analysis/gym_env.py; analysis/gym_en... | test/analysis/test_marl.py; pytest.importorskip("gymnasium")... | `roadmap.md` §A. |
...
| P13 | P13.4 | 24-hour soak test | L2 | test/test_performance.py; test/test_scaling.py | @slow; sys.getsizeof | `roadmap.md` §21. |
...
Provenance: the audit corpus documented in `roadmap.md`.*
