...
```bash
# All fast tests (marker-based: auto-skips GPU)
pytest test/ -v -m "not slow and not gl and not gpu"

# CI: fast suite — what runs on every commit
pytest test/ -v -m "not slow and not gl and not gpu" --timeout=60 --tb=short

# Guard rails (architecture, docs, golden, mode-switch, vicsek species) — run on every commit
pytest -m guard -v
...
pytest test/physics/forces/test_field.py -v -s

# Match by test name pattern
pytest test/ -k "test_vicsek" -v

# Performance & scaling (slow, nightly/PR-merge only)
pytest -m slow test/ -v

# Full suite including GPU (requires display / Xvfb)
pytest test/ -v
```
...
| `gpu` | Tests requiring GPU context (ModernGL) | `@pytest.mark.gpu` |
| `gl` | Tests requiring ModernGL GPU context (alias) | `@pytest.mark.gl` |
| `golden` | Golden trajectory regression tests | `@pytest.mark.golden` |
| `guard` | P14 guard-rail tests (architecture, docs, golden, mode-switch, vicsek species) | `@pytest.mark.guard` |
...
├── regenerate_golden.py        # P0.1 golden trajectory generator
├── core/
│   ├── test_types.py
│   └── test_config.py
├── simulation/
...
│   │   ├── test_forces.py
│   │   ├── test_projection.py
│   │   ├── test_spatial_variants.py
│   │   ├── test_field.py
│   │   ├── test_vicsek.py
│   │   ├── test_vicsek_core.py     # P1.8 memory term + tangent-plane noise
│   │   ├── test_vicsek_species.py  # P6 predator-prey
│   │   ├── test_influencer.py
│   │   ├── test_angle.py           # P5 (module-skipped until angle.py exists)
│   │   ├── test_force_terms.py
...
├── test_scaling.py
├── test_e2e.py
├── test_mode_switch_no_crash.py  # P14 guard: cycle all 7 modes
├── test_cli_e2e.py               # P10.5 skeleton: --set/--print-config/--fullscreen
├── test_imports.py
├── test_architecture.py          # P0.2 import rules enforcement
├── test_docs.py                  # P14 intra-repo link validation
├── test_golden.py                # P0.1 golden trajectory regression
├── test_config_drift.py          # P14 config-usage drift check
├── test_collection_count.py      # P14 test-collection guard
