...
class TestSubsystemB:
    ...
    def test_engine_config_live_mutation(self, default_config):
        """Mutating config.projection.phi_p between steps affects next step's forces."""
        from pymurmur.simulation.engine import SimulationEngine
        ...
        old_phi_p = sim.config.projection.phi_p
        sim.config.projection.phi_p = min(old_phi_p + 0.1, 1.0)

        sim.run_headless(steps=5)
        assert sim.config.projection.phi_p != old_phi_p
