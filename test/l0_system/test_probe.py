...
class TestProbeCapabilities:
    ...
    def test_probe_returns_valid_keys_regardless_of_deps(self):
        """P0.11: probe returns expected keys without crashing.

        Calls probe_capabilities() in the current environment — verifies
        all 7 expected keys exist and values are str|None. Does not mock
        imports; the function gracefully handles whatever deps are present.
        """
        caps = probe_capabilities()
