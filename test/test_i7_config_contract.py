...
from pymurmur.core.config import (
    SimConfig, _FIELD_MAP, _ALL_FIELD_NAMES, _DIRECT_FIELDS,
    DomainConfig, FlockConfig, BoundaryConfig, ProjectionConfig,
    SpatialConfig, FieldConfig, WanderConfig, VicsekConfig, InfluencerConfig,
    IndexConfig, RefinementConfig, ExtensionConfig, PredatorConfig,
...
def test_config_sub_classes_have_no_forbidden_imports():
    ...
    forbidden = {"pygame", "moderngl", "PIL", "numba", "matplotlib", "gymnasium",
                 "stable_baselines3"}
    config_src = (Path(__file__).parent.parent / "pymurmur" /
...
def test_config_yaml_roundtrip_preserves_all_sub_configs():
    ...
    cfg.alignment_radius_ratio = 0.5
    ...
    try:
        ...
        assert loaded.alignment_radius_ratio == 0.5
        assert loaded.use_toroidal_distance == False
        assert loaded.refinements == False
        assert loaded.steric == 0.3
        assert loaded.blind_deg == 90.0
        assert loaded.anisotropy == 3.0
        assert loaded.predator_enabled == True
        assert loaded.roosting_enabled == True
        assert loaded.wander_enabled == True
        assert loaded.ripple_enabled == False
        ...
        assert loaded.show_grid == True
        assert loaded.auto_rotate == True
        ...
        assert loaded.capture_with_viz == False
...
def test_public_facade_only_exports_intended_symbols():
    ...
    expected = {"SimConfig", "SimulationEngine", "Recorder"}
...
class TestFieldMapCompleteness:
    ...
    # All 16 sub-config classes (matching config.py __init__)
    ...
    def test_every_subconfig_field_has_field_map_entry(self):
        ...
        If a field is added to a dataclass but forgotten in _FIELD_MAP,
        ...
        for sub_attr, SubClass in self._SUBCONFIG_CLASSES.items():
            ...
            for f in fields(SubClass):
                ...
                assert found, (
                    ...
                    f"has no entry in _FIELD_MAP. Add it or it will "
                    f"silently default on YAML load."
                )
    ...
    def test_all_field_names_is_key_union(self):
        """IT4c: _ALL_FIELD_NAMES == _FIELD_MAP keys ∪ _DIRECT_FIELDS.

        If they diverge, from_file() filtering may drop fields or
        leave them unhandled.
        """
        expected = set(_FIELD_MAP.keys()) | _DIRECT_FIELDS
    ...
    def test_no_duplicate_field_map_entries(self):
        ...
        for flat_name, target in _FIELD_MAP.items():
    ...
    def test_roundtrip_subconfig_mutation_visible_via_flat_access(self):
        ...
        try:
            ...
            assert loaded.num_boids == 300, (
                f"num_boids lost in YAML round-trip"
            )
            assert loaded.boid_size == 7.5, (
                f"boid_size lost in YAML round-trip"
            )
...
class TestYamlKeyCollisionDetection:
    ...
    def test_yaml_section_keys_not_in_all_field_names(self):
        ...
        try:
            ...
            dropped = all_yaml_keys - _ALL_FIELD_NAMES
    ...
    def test_silently_dropped_extension_toggles(self):
        ...
        try:
            ...
            assert loaded.roosting_enabled == True, (
                f"BUG FIXED: roosting_enabled persisted!"
            )
            assert loaded.wander_enabled == True, (
                f"BUG FIXED: wander_enabled persisted!"
            )
            assert loaded.ripple_enabled == True, (
                f"BUG FIXED: ripple_enabled persisted!"
            )
        finally:
            tmp.unlink()
    ...
    def test_silently_dropped_capture_short_names(self):
        ...
        try:
            ...
            assert loaded.capture_frames == 888, (
                f"BUG FIXED: capture_frames persisted!"
            )
            ...
            assert loaded.capture_metrics_csv == "output/custom.csv", (
                f"BUG FIXED: capture_metrics_csv persisted!"
            )
            assert loaded.capture_metrics_json == "output/custom.json", (
                f"BUG FIXED: capture_metrics_json persisted!"
            )
            assert loaded.capture_with_viz == True, (
                f"BUG FIXED: capture_with_viz persisted!"
            )
        finally:
            tmp.unlink()
