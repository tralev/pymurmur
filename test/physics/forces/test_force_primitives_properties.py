...
class TestSeparationForceProperties:
    ...
    def test_magnitude_decreases_with_distance(self):
        """|F_sep| = 1/d for a single neighbour (code computes -diffs / dists^2).

        With one neighbour at distance d, diffs = [d, 0, 0] and
        |F| = |diffs| / d^2 = d / d^2 = 1/d.  Strictly decreases with d.
        """
...
class TestAlignmentForceProperties:
    ...
    def test_opposite_velocities_gives_max(self):
        """If all neighbours fly opposite, |F| → 2."""
        pos = np.array([[0, 0, 0], [5, 0, 0], [6, 0, 0]], dtype=np.float32)
        vel = np.array([[1, 0, 0], [-1, 0, 0], [-1, 0, 0]], dtype=np.float32)
        active = np.ones(3, dtype=bool)
        idx = np.array([[1, 2], [0, 2], [0, 1]], dtype=np.int32)
        f = alignment_force(pos, vel, idx, active)
        # Bird 0 sees birds 1 and 2 both flying left → avg is (-1, 0, 0)
        # û_avg = (-1, 0, 0), û_0 = (1, 0, 0) → force = (-2, 0, 0)
        assert np.allclose(f[0], [-2.0, 0.0, 0.0], atol=1e-5)
        assert np.allclose(np.linalg.norm(f[0]), 2.0, atol=1e-5)
...
class TestCohesionForceProperties:
    ...
    def test_force_is_unit_vector(self):
        """Per P1.7, cohesion returns a bounded unit vector (when nonzero)."""
        rng = _make_rng()
        for _ in range(30):
            N = rng.integers(2, 20)
            pos = rng.normal(scale=100, size=(N, 3)).astype(np.float32)
            ...
            for idx in (dense, ragged):
                ...
                if nonzero.any():
                    assert np.allclose(norms[nonzero], 1.0, atol=1e-5), (
                        f"Non-unit norms: {norms[nonzero]}"
                    )
...
class TestNoiseForceProperties:
    ...
    def test_unit_norm(self):
        """scale > 0 produces unit vectors on the sphere."""
        for scale in [0.1, 0.5, 1.0, 2.0, 10.0]:
            f = noise_force(500, scale)
            norms = np.linalg.norm(f, axis=1)
            assert np.allclose(norms, 1.0, atol=1e-5), (
                f"scale={scale}: non-unit norms, max diff={np.abs(norms - 1.0).max():.6f}"
            )
