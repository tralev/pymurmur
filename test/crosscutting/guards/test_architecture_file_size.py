"""File-size ceiling guard — no file under pymurmur/ exceeds 600 lines
unless explicitly documented in KNOWN_OVERSIZED_FILES.
...
def test_no_file_exceeds_line_limit():
    """Keep pymurmur/ modular -- every file should be small enough to
    read and reason about in one sitting (~600 lines). New files that
    grow past this should be split (see git history for the metrics.py/
    config.py/flock.py/evoflock.py/spatial.py/field.py splits for the
    established "pure extraction + re-export shim" pattern), not added
    to KNOWN_OVERSIZED_FILES -- that list is for pre-existing debt
    being paid down deliberately, not a place to grandfather new debt.
    """
    violations: list[str] = []
    known: list[str] = []
    ...
    if violations:
        msg = (
            ...
            f"guideline:\n"
            + "\n".join(violations)
            + "\n\nSplit the file (pure extraction + re-export shim, see "
            + "recent git history for the pattern), or if it's a deliberate, "
            + "temporary exception, add it to KNOWN_OVERSIZED_FILES with a reason.\n"
        )
        raise AssertionError(msg)

    print(
        f"✓ All pymurmur/ files within {MAX_FILE_LINES} lines "
