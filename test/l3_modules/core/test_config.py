...
def test_config_from_file_unknown_key_raises_valueerror():
    """G5: Unknown YAML keys raise ValueError naming the section and key."""
    import pytest
    import yaml

    # Top-level unknown key (not in a section) — still loaded as flat key
    # Section-level unknown key — should fail with actionable error
    data = {
        "mode": "vicsek",
        "flock": {"unknown_future_field": 42},
    }
    with tempfile.NamedTemporaryFile(suffix=".yaml", delete=False, mode="w") as f:
        yaml.dump(data, f)
        tmp = Path(f.name)
    ...
    try:
        with pytest.raises(ValueError, match="flock.*unknown_future_field"):
            SimConfig.from_file(tmp)
    finally:
        tmp.unlink()


def test_config_from_file_actionable_error_names_section_and_key():
    """G5: ValueError message contains both section name and offending key."""
    import pytest
    import yaml

    data = {
        "mode": "projection",
        "spatial": {"typo_weight": 1.0},
    }
    with tempfile.NamedTemporaryFile(suffix=".yaml", delete=False, mode="w") as f:
        yaml.dump(data, f)
        tmp = Path(f.name)
    ...
    try:
        ...
        assert "spatial" in msg, f"Error must name the section, got: {msg}"
        assert "typo_weight" in msg, f"Error must name the offending key, got: {msg}"
    finally:
        tmp.unlink()


def test_config_from_file_known_keys_loaded():
    """Known keys in a section still load correctly alongside unknown ones."""
    import yaml

    data = {
        "mode": "vicsek",
        "flock": {"num_boids": 300},
        # Top-level unknown key — this is treated as a flat key
        # and gets filtered. No section context = no error.
    }
    with tempfile.NamedTemporaryFile(suffix=".yaml", delete=False, mode="w") as f:
        yaml.dump(data, f)
        tmp = Path(f.name)
...
def test_field_map_routes_all_fields_correctly():
    ...
    from pymurmur.core.config import _DIRECT_FIELDS, _FIELD_MAP
    cfg = SimConfig()
...
def test_setattr_routes_to_sub_config_not_simconfig():
    """config.phi_p routes to _projection.phi_p, not SimConfig.__dict__."""
    cfg = SimConfig()
    ...
    assert cfg._extension.predator_enabled
...
def test_config_equality_compares_all_fields():
    """__eq__ compares all sub-configs and direct fields for equality."""
    cfg1 = SimConfig()
    cfg2 = SimConfig()
    ...
    # Nested-only field (flat shim retired) is compared too
    cfg5 = SimConfig()
    cfg5.projection.phi_p = 0.19
    assert cfg5 != cfg1, "Different projection.phi_p must make configs unequal"
